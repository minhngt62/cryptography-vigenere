...
class KasiskiMEthod:
    ...
    def getNGramPositions(self, nGram, text):
    ...
    def estimateKeyLength(self, cipheredText):
        def estimateKeyLength(cipheredText, maxSamples):
            tetraGrams = self.getNGrams(cipheredText, 4)
            triGrams = self.getNGrams(cipheredText, 3)
            positions = []
            i = 0
            j = 0
            while len(positions) < maxSamples: 
                if tetraGrams[i][1] >= 2:
                    positions.append(self.getNGramPositions(tetraGrams[i][0], cipheredText))
                    i+=1
                elif triGrams[i][1] >= 2:
                    positions.append(self.getNGramPositions(triGrams[j][0], cipheredText))
                    j+=1
                else:
                    break
            differences = []
